    ...
    decode(substr(c.name,1,3),
      '***','* '||
        decode(substr(c.name,17,2),
          ' 1',
'Parametercheck for Oracle 11.2 based on Note/Version: 1431798/78',
          ' 2',
'Parametercheck last changed: 2013-12-16',
          ' 3',
...
||'_B_TREE_BITMAP_PLANS#-man-set to FALSE if Winbundle<=5#'
  ||'PS[1], OSF[WINDOWS]#2p#'
  ||'avoid bitmap operations when using B*TREE indexes#'
||'_BUG16850197_ENABLE_FIX_FOR_13602883#1#PS[3], OSF[UNIX], MF[12]#2f#'
...
||'_FIX_CONTROL,12693573#12693573:OFF#EXADATA[TRUE]#2p#'
...
||'_FIX_CONTROL,14255600#-man-set to 14255600:ON if Winbundle>=25#'
  ||'PS[2], OSF[WINDOWS]#2p#'
  ||'statistic collection during index creation#'
||'_FIX_CONTROL,14595273#14595273:ON#PS[3], OSF[UNIX]#2p##'
...
||'_SECOND_SPARE_PARAMETER#1#PS[1], OSF[UNIX], MF[3-99]#2p#'
  ||'Avoid high CPU consumption for Mutex requests#'
||'_SECUREFILES_CONCURRENCY_ESTIMATE#50#PS[3]#2p#'
...
'C#16470836#13#C#16976121#12#'||
'C#16092378#11#C#14712222#10#C#14723910#9#C#13891981#8#C#14467202#7#C#14255600#6#'||
...
'=121110=130210=130310=130610=130810=131110=131210' mf_date
          from dual )
        ) dt
        where
      ...
      (
        ...
        (
          ...
          (
            ...
            from
            ( select '#'||
'_B_TREE_BITMAP_PLANS#_BUG16850197_ENABLE_FIX_FOR_13602883#'||
'_ENABLE_NUMA_SUPPORT#'||
...
'_IN_MEMORY_UNDO#_MUTEX_WAIT_SCHEME#_MUTEX_WAIT_TIME#_NINTH_SPARE_PARAMETER#'||
